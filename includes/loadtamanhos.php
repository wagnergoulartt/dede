										...
										<form id="formeditartamanho_<?=$id_option;?>" style="margin-top: 10px;" method="post">
											<div>
												<div class="form-group">
													...
													<label for="edit_valor_tamanho">VALOR R$ <small style="color: red;">* Ignore esse campo no caso de meio a meio.</small></label>
													<input required
													type="text" 
				...
				$.ajax({
					...
					success: function(data){
						...
						if(data == 'erro1'){
							...
							x0p('Sucesso!', 
								'Tamanho de produto Atualizado!', 
								'ok', false);
							$('.table-responsive').load('<?=$site;?>includes/loadtamanhos.php', {'user_id' : '<?=$_POST['user_id'];?>', 'tipo_id' : '<?=$_POST['tipo_id'];?>'});
						}else if(data == 'false'){
							x0p('Opss...', 
								'OCORREU UM ERRO!',
								'error', false);
